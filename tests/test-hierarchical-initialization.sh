...
# =============================================================================
# Test 21: Custom Path - Complete Hierarchy with .myorg
# =============================================================================
test_section "Custom Path - Complete Hierarchy with .myorg"

setup_test_env
export RCM_CLAUDE_DIR_NAME=".myorg"

# Create company with custom path
mkdir -p "$TEST_TMP/custom-acme/.myorg"
save_level_with_hierarchy "$TEST_TMP/custom-acme/.myorg" "company" "custom-acme" >/dev/null 2>&1

if [[ -f "$TEST_TMP/custom-acme/.myorg/organizational-level.json" ]]; then
    test_pass "Custom path: Company level created with .myorg"
else
    test_fail "Custom path: Company level not created"
fi

# Create product under custom company
mkdir -p "$TEST_TMP/custom-acme/custom-widget/.myorg"
save_level_with_hierarchy "$TEST_TMP/custom-acme/custom-widget/.myorg" "product" "custom-widget" >/dev/null 2>&1

if [[ -f "$TEST_TMP/custom-acme/custom-widget/.myorg/organizational-level.json" ]]; then
    test_pass "Custom path: Product level created with .myorg"

    PARENT=$(jq -r '.parent_claude_dir' "$TEST_TMP/custom-acme/custom-widget/.myorg/organizational-level.json")
    if [[ "$PARENT" == "$TEST_TMP/custom-acme/.myorg" ]]; then
        test_pass "Custom path: Product references custom company parent"
    else
        test_fail "Custom path: Product parent reference incorrect: $PARENT"
    fi
else
    test_fail "Custom path: Product level not created"
fi

# Create project under custom product
mkdir -p "$TEST_TMP/custom-acme/custom-widget/custom-api/.myorg"
save_level_with_hierarchy "$TEST_TMP/custom-acme/custom-widget/custom-api/.myorg" "project" "custom-api" >/dev/null 2>&1

if [[ -f "$TEST_TMP/custom-acme/custom-widget/custom-api/.myorg/organizational-level.json" ]]; then
    test_pass "Custom path: Project level created with .myorg"

    HIERARCHY_STR=$(jq -r '.hierarchy_path | join(",")' "$TEST_TMP/custom-acme/custom-widget/custom-api/.myorg/organizational-level.json")
    if [[ "$HIERARCHY_STR" == "company,product,project" ]]; then
        test_pass "Custom path: Complete hierarchy path correct"
    else
        test_fail "Custom path: Hierarchy path incorrect: $HIERARCHY_STR"
    fi
else
    test_fail "Custom path: Project level not created"
fi

unset RCM_CLAUDE_DIR_NAME

# =============================================================================
# Test 22: Custom Path - Mixed Hierarchy (.claude + custom)
# =============================================================================
test_section "Custom Path - Mixed Hierarchy"

setup_test_env

# Create company with .claude
mkdir -p "$TEST_TMP/mixed-company/.claude"
save_level_with_hierarchy "$TEST_TMP/mixed-company/.claude" "company" "mixed-company" >/dev/null 2>&1

# Switch to custom path for product
export RCM_CLAUDE_DIR_NAME=".custom"
mkdir -p "$TEST_TMP/mixed-company/mixed-product/.custom"
save_level_with_hierarchy "$TEST_TMP/mixed-company/mixed-product/.custom" "product" "mixed-product" >/dev/null 2>&1

if [[ -f "$TEST_TMP/mixed-company/mixed-product/.custom/organizational-level.json" ]]; then
    test_pass "Mixed hierarchy: Product created with .custom"

    PARENT=$(jq -r '.parent_claude_dir' "$TEST_TMP/mixed-company/mixed-product/.custom/organizational-level.json")
    if [[ "$PARENT" == "$TEST_TMP/mixed-company/.claude" ]]; then
        test_pass "Mixed hierarchy: Custom child finds .claude parent"
    else
        test_fail "Mixed hierarchy: Parent reference incorrect: $PARENT"
    fi
else
    test_fail "Mixed hierarchy: Product not created"
fi

unset RCM_CLAUDE_DIR_NAME

# =============================================================================
# Test 23: Custom Path - Custom Role Guides Directory
# =============================================================================
test_section "Custom Path - Custom Role Guides Directory"

setup_test_env
export RCM_CLAUDE_DIR_NAME=".testorg"
export RCM_ROLE_GUIDES_DIR="my-guides"

mkdir -p "$TEST_TMP/guides-company/.testorg"
save_level_with_hierarchy "$TEST_TMP/guides-company/.testorg" "company" "guides-company" >/dev/null 2>&1

if [[ -f "$TEST_TMP/guides-company/.testorg/organizational-level.json" ]]; then
    test_pass "Custom guides: Company created with custom paths"

    # Verify we can read the level back
    LEVEL=$(get_level_value "$TEST_TMP/guides-company/.testorg")
    if [[ "$LEVEL" == "company" ]]; then
        test_pass "Custom guides: Can read level from custom directory"
    else
        test_fail "Custom guides: Cannot read level: $LEVEL"
    fi
else
    test_fail "Custom guides: Company not created"
fi
...
# =============================================================================
# Test 24: Custom Path - Validation with Custom Paths
# =============================================================================
test_section "Custom Path - Validation"

setup_test_env
export RCM_CLAUDE_DIR_NAME=".validate"

# Create valid hierarchy
mkdir -p "$TEST_TMP/valid-company/.validate"
save_level_with_hierarchy "$TEST_TMP/valid-company/.validate" "company" "valid-company" >/dev/null 2>&1

mkdir -p "$TEST_TMP/valid-company/valid-product/.validate"
save_level_with_hierarchy "$TEST_TMP/valid-company/valid-product/.validate" "product" "valid-product" >/dev/null 2>&1

cd "$TEST_TMP/valid-company/valid-product"
if validate_hierarchy "$PWD" >/dev/null 2>&1; then
    test_pass "Custom path: validate_hierarchy works with custom paths"
else
    test_fail "Custom path: validate_hierarchy failed with custom paths"
fi

# Test invalid hierarchy with custom paths
mkdir -p "$TEST_TMP/invalid-project/.validate"
cat > "$TEST_TMP/invalid-project/.validate/organizational-level.json" <<EOF
{
  "level": "project",
  "level_name": "invalid-project",
  "parent_claude_dir": null,
  "parent_level": null,
  "is_root": true,
  "hierarchy_path": ["project"]
...
mkdir -p "$TEST_TMP/invalid-project/invalid-child/.validate"
cat > "$TEST_TMP/invalid-project/invalid-child/.validate/organizational-level.json" <<EOF
{
  "level": "product",
  "level_name": "invalid-child",
  "parent_claude_dir": "$TEST_TMP/invalid-project/.validate",
  "parent_level": "project",
  "is_root": false,
  "hierarchy_path": ["project", "product"]
}
...
cd "$TEST_TMP/invalid-project/invalid-child"
if validate_hierarchy "$PWD" >/dev/null 2>&1; then
    test_fail "Custom path: Should detect invalid hierarchy (product under project)"
else
    test_pass "Custom path: Correctly detected invalid hierarchy"
fi

unset RCM_CLAUDE_DIR_NAME

# =============================================================================
# Test 25: Custom Path - Performance
# =============================================================================
test_section "Custom Path - Performance"

setup_test_env
export RCM_CLAUDE_DIR_NAME=".perf"

# Create 4-level hierarchy
mkdir -p "$TEST_TMP/perf-company/.perf"
save_level_with_hierarchy "$TEST_TMP/perf-company/.perf" "company" "perf-company" >/dev/null 2>&1

mkdir -p "$TEST_TMP/perf-company/perf-system/.perf"
save_level_with_hierarchy "$TEST_TMP/perf-company/perf-system/.perf" "system" "perf-system" >/dev/null 2>&1

mkdir -p "$TEST_TMP/perf-company/perf-system/perf-product/.perf"
save_level_with_hierarchy "$TEST_TMP/perf-company/perf-system/perf-product/.perf" "product" "perf-product" >/dev/null 2>&1

mkdir -p "$TEST_TMP/perf-company/perf-system/perf-product/perf-project/.perf"

# Time the initialization
START=$(date +%s%N)
save_level_with_hierarchy "$TEST_TMP/perf-company/perf-system/perf-product/perf-project/.perf" "project" "perf-project" >/dev/null 2>&1
END=$(date +%s%N)
ELAPSED_MS=$(( (END - START) / 1000000 ))

if [[ "$ELAPSED_MS" -lt 200 ]]; then
    test_pass "Custom path: Initialization with hierarchy detection <200ms ($ELAPSED_MS ms)"
else
    test_fail "Custom path: Initialization too slow ($ELAPSED_MS ms)"
fi

# Verify correctness
HIERARCHY=$(jq -r '.hierarchy_path | join(",")' "$TEST_TMP/perf-company/perf-system/perf-product/perf-project/.perf/organizational-level.json")
if [[ "$HIERARCHY" == "company,system,product,project" ]]; then
    test_pass "Custom path: 4-level hierarchy correct after initialization"
else
    test_fail "Custom path: Hierarchy incorrect: $HIERARCHY"
fi
...
if [[ $TESTS_FAILED -eq 0 ]]; then
    ...
    echo "  ✓ Custom path configuration support"
    exit 0
else
