...
# =============================================================================
# Test 25: Custom Path - Hierarchy with .myorg directory
# =============================================================================
test_section "Custom Path - Hierarchy with .myorg"

setup_test_env
export RCM_CLAUDE_DIR_NAME=".myorg"

# Create three-level hierarchy with custom directory name
mkdir -p "$TEST_TMP/custom-level1/.myorg"
mkdir -p "$TEST_TMP/custom-level1/custom-level2/.myorg"
mkdir -p "$TEST_TMP/custom-level1/custom-level2/custom-level3"

# Test find_parent_claude_dirs with custom paths
RESULT=$(find_parent_claude_dirs "$TEST_TMP/custom-level1/custom-level2/custom-level3")
COUNT=$(echo "$RESULT" | wc -l)

if [[ "$COUNT" -eq 2 ]]; then
    test_pass "Custom path: Found correct number of parent directories (2)"
else
    test_fail "Custom path: Found incorrect number of parent directories (expected: 2, got: $COUNT)"
fi

# Verify nearest parent with custom directory name
FIRST_PARENT=$(echo "$RESULT" | head -n1)
if [[ "$FIRST_PARENT" == "$TEST_TMP/custom-level1/custom-level2/.myorg" ]]; then
    test_pass "Custom path: Nearest parent is first in list"
else
    test_fail "Custom path: Nearest parent not first (got: $FIRST_PARENT)"
fi

unset RCM_CLAUDE_DIR_NAME

# =============================================================================
# Test 26: Custom Path - Mixed hierarchy (.claude parent, custom child)
# =============================================================================
test_section "Custom Path - Mixed Hierarchy"

setup_test_env
mkdir -p "$TEST_TMP/mixed-parent/.claude"
cat > "$TEST_TMP/mixed-parent/.claude/organizational-level.json" <<'EOF'
{"level": "company", "is_root": true}
EOF

# Now use custom path for child
export RCM_CLAUDE_DIR_NAME=".custom"
mkdir -p "$TEST_TMP/mixed-parent/mixed-child/.custom"

# Test that we can still find .claude parent from custom child
RESULT=$(get_nearest_parent "$TEST_TMP/mixed-parent/mixed-child")
if [[ "$RESULT" == "$TEST_TMP/mixed-parent/.claude" ]]; then
    test_pass "Mixed hierarchy: Found .claude parent from custom child"
else
    test_fail "Mixed hierarchy: Did not find .claude parent (got: $RESULT)"
fi

unset RCM_CLAUDE_DIR_NAME

# =============================================================================
# Test 27: Custom Path - Role guides directory customization
# =============================================================================
...
mkdir -p "$TEST_TMP/custom-guides/.myorg/guides"
cat > "$TEST_TMP/custom-guides/.myorg/organizational-level.json" <<'EOF'
{"level": "company", "level_name": "Test", "is_root": true}
EOF

# Verify the organizational-level.json exists and can be read
LEVEL=$(get_level_value "$TEST_TMP/custom-guides/.myorg")
if [[ "$LEVEL" == "company" ]]; then
    test_pass "Custom guides: Can read level from custom directory"
else
    test_fail "Custom guides: Cannot read level (got: $LEVEL)"
fi
...
# =============================================================================
# Test 28: Custom Path - save_level_with_hierarchy with custom paths
# =============================================================================
test_section "Custom Path - save_level_with_hierarchy"

setup_test_env
export RCM_CLAUDE_DIR_NAME=".testorg"

mkdir -p "$TEST_TMP/save-custom-company/.testorg"
save_level_with_hierarchy "$TEST_TMP/save-custom-company/.testorg" "company" "Custom Company" >/dev/null 2>&1

if [[ -f "$TEST_TMP/save-custom-company/.testorg/organizational-level.json" ]]; then
    test_pass "Custom path: save_level_with_hierarchy creates file in custom directory"

    LEVEL=$(jq -r '.level' "$TEST_TMP/save-custom-company/.testorg/organizational-level.json")
    if [[ "$LEVEL" == "company" ]]; then
        test_pass "Custom path: Saved level is correct"
    else
        test_fail "Custom path: Saved level is incorrect (got: $LEVEL)"
    fi
else
    test_fail "Custom path: save_level_with_hierarchy did not create file"
fi

# Create child with custom path
mkdir -p "$TEST_TMP/save-custom-company/save-custom-product/.testorg"
save_level_with_hierarchy "$TEST_TMP/save-custom-company/save-custom-product/.testorg" "product" "Custom Product" >/dev/null 2>&1

if [[ -f "$TEST_TMP/save-custom-company/save-custom-product/.testorg/organizational-level.json" ]]; then
    test_pass "Custom path: save_level_with_hierarchy creates child file in custom directory"

    PARENT=$(jq -r '.parent_claude_dir' "$TEST_TMP/save-custom-company/save-custom-product/.testorg/organizational-level.json")
    if [[ "$PARENT" == "$TEST_TMP/save-custom-company/.testorg" ]]; then
        test_pass "Custom path: Child references custom parent correctly"
    else
        test_fail "Custom path: Child parent reference incorrect (got: $PARENT)"
    fi
else
    test_fail "Custom path: save_level_with_hierarchy did not create child file"
fi

unset RCM_CLAUDE_DIR_NAME

# =============================================================================
# Test 29: Custom Path - Performance with custom paths
# =============================================================================
...
mkdir -p "$TEST_TMP/perf-test/l1/.perf"
mkdir -p "$TEST_TMP/perf-test/l1/l2/.perf"
mkdir -p "$TEST_TMP/perf-test/l1/l2/l3/.perf"
mkdir -p "$TEST_TMP/perf-test/l1/l2/l3/l4/.perf"
mkdir -p "$TEST_TMP/perf-test/l1/l2/l3/l4/l5/.perf"

# Time the parent finding operation
START=$(date +%s%N)
PARENTS=$(find_parent_claude_dirs "$TEST_TMP/perf-test/l1/l2/l3/l4/l5")
END=$(date +%s%N)
ELAPSED_MS=$(( (END - START) / 1000000 ))

if [[ "$ELAPSED_MS" -lt 100 ]]; then
    test_pass "Custom path: find_parent_claude_dirs completes in <100ms ($ELAPSED_MS ms)"
else
    test_fail "Custom path: find_parent_claude_dirs too slow ($ELAPSED_MS ms)"
fi

PARENT_COUNT=$(echo "$PARENTS" | wc -l)
if [[ "$PARENT_COUNT" -eq 4 ]]; then
    test_pass "Custom path: Found all 4 parents in deep hierarchy"
else
    test_fail "Custom path: Parent count incorrect (expected 4, got: $PARENT_COUNT)"
fi
